...
# }
birthdays = {
    (datarow.month, datarow.day)                # tuple from pandas.Series
    :
    (datarow)                                   # complete panda.Series
    for (index, datarow) in data.iterrows()     # extract index and Series from each row, ignore index
}
...
if today in birthdays:
    ...
    with open(letter_file, "r") as fp:
        letter_text = fp.read()
        letter = letter_text.replace("[NAME]", birthday['name'])
        print(letter)
